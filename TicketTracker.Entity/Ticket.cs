...
namespace TicketTracker.Entity
{
    public class Ticket
    {
        ...
        public Ticket()
        : this(TicketId.Default, string.Empty, string.Empty, TicketAttribute.Create(ticketType: TicketType.Default), null,
        ...
        {
            return Create(TicketId.Default, title, description, TicketAttribute.Create(ticketType: TicketType.Default), null,
    ...
    public record TicketAttribute(List<Label>? Labels, TicketType? TicketType, Priority Priority, ushort EstimationPoint, DateTime? DueDate)
    {
        public List<Label>? Labels { get; private set; } = Labels;

        public TicketType? TicketType { get; private set; } = TicketType;

        public Priority Priority { get; private set; } = Priority;

        public ushort EstimationPoint { get; private set; } = EstimationPoint;

        public DateTime? DueDate { get; private set; } = DueDate;

        internal static TicketAttribute Create(List<Label>? labels = null, TicketType? ticketType = null, Priority? priority = null,
            ushort estimationPoint = 0, DateTime? dueDate = null)
            => new(labels ?? new List<Label>(), ticketType, priority ?? Priority.Default, estimationPoint, dueDate);
    ...
    public record Comment(Guid Id, string Content, AccountId? CreatorId, AccountId? EditorIdId, DateTime? CreateDate, DateTime? ModifiedDate)
    {
        #region Factory Methods
    ...
    public record TicketTimeInfo(DateTime CreateDate, DateTime? ModifiedDate = null, DateTime? EndDate = null, DateTime? LeadTime = null)
    {
        public TicketTimeInfo Update() => new(CreateDate, DateTimer.Now);
